...
#
# Sourced by the `pila` launcher's normal post-run path and by the
# `pila --finalize <run-id>` fast-path. Both code paths share the same
# push/PR mechanics — the discovery of *which* run to finalize differs.
#
# Exports: host_finalize <run-dir>
#
# Inputs (env or args):
#   $1                — absolute path to .pila/runs/<run-id>/
...
host_finalize() {
  ...
  if [ "$(jq -r '.no_push // false' "$run_json")" = "true" ]; then
    echo "[pila] finalize: run.json has no_push=true; skipping push + PR" >&2
    return 0
  fi

  # Honor already-pushed (idempotent re-invocation of --finalize).
  if [ -n "$(jq -r '.pushed_at // ""' "$run_json")" ]; then
    echo "[pila] finalize: run already pushed; nothing to do" >&2
    return 0
  fi
  ...
  if [ -z "$run_branch" ] || [ -z "$working_branch" ]; then
    echo "pila: error — run.json at $run_json is missing branch info." >&2
  ...
  echo "[pila] finalize: pushing $run_branch to origin$([ "${NO_VERIFY_PUSH:-false}" = "true" ] && echo " (--no-verify)")" >&2
  local push_stderr
