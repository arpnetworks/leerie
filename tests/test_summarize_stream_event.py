...
def test_extract_text_from_string_content(centella):
    """When tool_result.content is a plain string."""
    block = {"type": "tool_result", "content": "the file says hello"}
    assert centella._extract_tool_result_text(block) == "the file says hello"


def test_extract_text_from_list_content(centella):
    """When tool_result.content is a list of {type:'text', text:'...'}."""
    block = {"type": "tool_result", "content": [
        {"type": "text", "text": "line 1"},
        {"type": "text", "text": "line 2"},
    ]}
    assert centella._extract_tool_result_text(block) == "line 1 line 2"


def test_extract_text_handles_missing_content(centella):
    assert centella._extract_tool_result_text({}) == ""
    assert centella._extract_tool_result_text({"content": None}) == ""


# ----- _summarize_tool_use: per-tool shape ----------------------------------

def test_read_tool(centella):
    block = {"type": "tool_use", "name": "Read",
             "input": {"file_path": "src/foo.py"}}
    assert centella._summarize_tool_use("x", block, "stream") == "  [x read] src/foo.py"


def test_grep_tool_with_path(centella):
    block = {"type": "tool_use", "name": "Grep",
             "input": {"pattern": "TODO", "path": "src/"}}
    assert centella._summarize_tool_use("x", block, "stream") == "  [x grep] TODO in src/"


def test_grep_tool_without_path(centella):
    block = {"type": "tool_use", "name": "Grep", "input": {"pattern": "TODO"}}
    assert centella._summarize_tool_use("x", block, "stream") == "  [x grep] TODO"


def test_bash_tool_not_truncated_at_stream(centella):
    ...
    out = centella._summarize_tool_use("x", block, "stream")
    # Full command preserved; only the leading prefix `  [x bash] ` is added.
    assert out == f"  [x bash] {long_cmd}"


def test_bash_tool_stream_and_debug_identical(centella):
    ...
    block = {"type": "tool_use", "name": "Bash",
             "input": {"command": long_cmd}}
    out_stream = centella._summarize_tool_use("x", block, "stream")
    out_debug = centella._summarize_tool_use("x", block, "debug")
    assert out_stream == out_debug


def test_bash_tool_first_line_only(centella):
    block = {"type": "tool_use", "name": "Bash",
...
def test_write_edit_tools(centella):
...
def test_structured_output_tool_stream_suppressed(centella):
...
def test_structured_output_not_truncated_at_debug(centella):
...
def test_unknown_tool_falls_through(centella):
...
def test_unknown_tool_not_truncated(centella):
...
def test_tool_use_handles_missing_input(centella):
...
def test_quiet_drops_assistant_events(centella):
    event = {"type": "assistant",
             "message": {"content": [{"type": "text", "text": "hi"}]}}
    assert centella._summarize_stream_event("x", event, "quiet") is None


def test_normal_drops_assistant_events(centella):
    event = {"type": "assistant",
             "message": {"content": [{"type": "text", "text": "hi"}]}}
    assert centella._summarize_stream_event("x", event, "normal") is None


def test_quiet_surfaces_worker_errors(centella):
    ...
    out = centella._summarize_stream_event("x", event, "quiet")
    assert out is not None
    assert "worker failed" in out
    assert "error_max_turns" in out


def test_normal_surfaces_worker_errors(centella):
    event = {"type": "result", "subtype": "error_max_turns",
             "is_error": True, "num_turns": 5}
    assert centella._summarize_stream_event("x", event, "normal") is not None


# ----- system/init at stream and above --------------------------------------

def test_system_init_at_stream(centella):
    """Captured shape: {type:'system', subtype:'init', model:'...', ...}.
    Stream level shows model name; full payload goes to the file."""
    event = {"type": "system", "subtype": "init",
             "model": "claude-opus-4-7[1m]"}
    out = centella._summarize_stream_event("x", event, "stream")
    assert out is not None
    assert "starting" in out
    assert "claude-opus-4-7[1m]" in out


def test_hook_events_dropped_at_stream(centella):
    ...
    assert centella._summarize_stream_event("x", event, "stream") is None


def test_hook_events_visible_at_debug(centella):
    event = {"type": "system", "subtype": "hook_started",
             "hook_name": "SessionStart:startup"}
    out = centella._summarize_stream_event("x", event, "debug")
    assert out is not None
...
def test_assistant_text_block_at_stream(centella):
...
def test_assistant_text_block_all_lines_emitted(centella):
...
def test_assistant_text_block_not_truncated(centella):
...
def test_assistant_tool_use_block(centella):
    event = {"type": "assistant", "message": {
...
def test_assistant_multiple_blocks(centella):
...
def test_assistant_structured_output_with_text_no_none_literal(centella):
...
def test_assistant_empty_content_returns_none(centella):
...
def test_tool_failure_surfaces_at_stream(centella):
...
def test_tool_failure_not_truncated(centella):
...
def test_tool_success_dropped_at_stream(centella):
...
def test_tool_success_visible_at_debug(centella):
    event = {"type": "user", "message": {"content": [
        {"type": "tool_result", "is_error": False,
         "content": "file contents here",
         "tool_use_id": "tu_1"},
...
def test_tool_failure_multiline_each_line_tagged(centella):
...
def test_tool_success_multiline_each_line_tagged_at_debug(centella):
...
def test_tool_success_not_truncated_at_debug(centella):
...
def test_rate_limit_threshold_crossing_surfaces(centella):
...
def test_rate_limit_quiet_event_dropped_at_stream(centella):
...
def test_rate_limit_always_visible_at_debug(centella):
    event = {"type": "rate_limit_event", "rate_limit_info": {
        "status": "allowed", "utilization": 0.10,
    }}
    assert centella._summarize_stream_event("x", event, "debug") is not None


# ----- result envelope -----------------------------------------------------

def test_result_success_summary(centella):
...
def test_result_error_summary_at_stream(centella):
...
def test_unknown_event_type_at_debug(centella):
...
def test_unknown_event_type_dropped_at_stream(centella):
    event = {"type": "future_event_type", "subtype": "novel"}
    assert centella._summarize_stream_event("x", event, "stream") is None
