...
docs — those repos route through `.leerie-setup.sh`. The fallback chain
...
def test_atx_header_install_section(leerie):
...
def test_setext_h1_header(leerie):
...
def test_setext_h2_header(leerie):
...
def test_asciidoc_double_equals_header(leerie):
...
def test_emoji_prefix_header_still_matches(leerie):
...
def test_bullet_prefix_header(leerie):
    text = (
        "# Cool\n\nIntro.\n\n"
        "## • Install\n\n"
        "Run `cargo build`.\n"
...
def test_section_keyword_coverage(leerie, header, install_token):
...
def test_intro_kept_under_1kb(leerie):
...
def test_long_intro_truncated_to_budget(leerie):
...
def test_no_header_falls_back_to_code_fence(leerie):
...
def test_no_header_no_fence_falls_back_to_top_6kb(leerie):
...
def test_text_with_only_header_no_body(leerie):
