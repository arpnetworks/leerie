...
Covers the precedence order: CLI flag → LEERIE_NO_PUSH env var →
no_push in leerie.toml → False (push by default per DESIGN §6).
...
def repo_root(tmp_path, monkeypatch):
    """An empty repo-root directory with LEERIE_NO_PUSH unset."""
    monkeypatch.delenv("LEERIE_NO_PUSH", raising=False)
    return tmp_path


def test_default_is_push_enabled(leerie, repo_root):
    """No CLI flag, no env, no file → False (push by default)."""
    assert leerie.resolve_no_push(repo_root, cli_value=False) is False


def test_cli_flag_wins(leerie, repo_root, monkeypatch):
    """--no-push CLI flag is the highest precedence."""
    monkeypatch.setenv("LEERIE_NO_PUSH", "0")
    (repo_root / "leerie.toml").write_text("no_push = false\n")
    assert leerie.resolve_no_push(repo_root, cli_value=True) is True


def test_env_set_true(leerie, repo_root, monkeypatch):
    monkeypatch.setenv("LEERIE_NO_PUSH", "1")
    assert leerie.resolve_no_push(repo_root, cli_value=False) is True
...
def test_env_set_false_falls_through_to_default(leerie, repo_root, monkeypatch):
    ...
    monkeypatch.setenv("LEERIE_NO_PUSH", "false")
    assert leerie.resolve_no_push(repo_root, cli_value=False) is False


def test_file_set_true_no_env(leerie, repo_root):
    (repo_root / "leerie.toml").write_text("no_push = true\n")
    assert leerie.resolve_no_push(repo_root, cli_value=False) is True
...
def test_env_truthy_spellings(leerie, repo_root, monkeypatch, value):
    monkeypatch.setenv("LEERIE_NO_PUSH", value)
    assert leerie.resolve_no_push(repo_root, cli_value=False) is True
...
def test_env_garbage_dies(leerie, repo_root, monkeypatch):
    ...
    monkeypatch.setenv("LEERIE_NO_PUSH", "maybe")
    with pytest.raises(SystemExit):
        leerie.resolve_no_push(repo_root, cli_value=False)


def test_file_garbage_dies(leerie, repo_root):
    (repo_root / "leerie.toml").write_text("no_push = sometimes\n")
    with pytest.raises(SystemExit):
        leerie.resolve_no_push(repo_root, cli_value=False)


def test_env_empty_string_falls_through(leerie, repo_root, monkeypatch):
    """LEERIE_NO_PUSH="" should be treated as unset, not as a value."""
    monkeypatch.setenv("LEERIE_NO_PUSH", "")
    assert leerie.resolve_no_push(repo_root, cli_value=False) is False
