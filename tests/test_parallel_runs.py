...
leerie runs in the same repository have completely disjoint state.
...
def test_disjoint_state_paths(leerie, tmp_path):
...
def test_disjoint_run_dirs(leerie, tmp_path):
    """Run dirs are siblings under leerie_root/runs/; nothing inside
...
def test_disjoint_subpaths(leerie, tmp_path):
    """The per-run subdirectories that leerie code uses
    (leerie_dir / 'worktrees', '/criteria', '/logs', '/subtasks',
...
def test_concurrent_saves_do_not_clobber(leerie, tmp_path):
...
def test_interleaved_saves_do_not_clobber(leerie, tmp_path):
...
def test_same_subtask_id_in_different_runs(leerie, tmp_path):
