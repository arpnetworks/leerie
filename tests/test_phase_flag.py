...
def _build_parser(leerie) -> argparse.ArgumentParser:
    """Reconstruct the argument parser from the leerie module.
    ...
    Since leerie.py embeds its parser inside main() with no factory
...
def test_phase_judge_parses():
    ...
    result = subprocess.run(
        [sys.executable, str(LEERIE_PY), "--phase", "judge", "--help"],
...
def test_phase_heal_parses():
    ...
    result = subprocess.run(
        [sys.executable, str(LEERIE_PY), "--phase", "heal", "--help"],
        capture_output=True, text=True,
    )
    assert result.returncode == 0, result.stderr
...
def test_phase_invalid_rejected():
    ...
    result = subprocess.run(
        [sys.executable, str(LEERIE_PY), "--phase", "invalid"],
        capture_output=True, text=True,
    )
...
def test_no_phase_accepted():
    ...
    result = subprocess.run(
        [sys.executable, str(LEERIE_PY), "--help"],
        capture_output=True, text=True,
    )
    assert result.returncode == 0
...
# ---------------------------------------------------------------------------
# Programmatic argparse introspection via the leerie module's known parser
...
def test_phase_choices_are_judge_and_heal(leerie):
    ...
    assert hasattr(leerie, "phase_judge"), "phase_judge not found in leerie"
    assert callable(leerie.phase_judge)
    assert hasattr(leerie, "phase_heal"), "phase_heal not found in leerie"
