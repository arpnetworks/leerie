...
def test_sanitize_slug_typical(leerie):
    assert leerie._sanitize_slug("Fix the login timeout bug") == "fix-the-login-timeout-bug"


def test_sanitize_slug_lowercases(leerie):
    assert leerie._sanitize_slug("ALL CAPS TASK") == "all-caps-task"


def test_sanitize_slug_collapses_repeated_dashes(leerie):
    assert leerie._sanitize_slug("foo---bar___baz") == "foo-bar-baz"


def test_sanitize_slug_strips_leading_trailing(leerie):
    assert leerie._sanitize_slug("---foo---") == "foo"
    assert leerie._sanitize_slug("...foo...") == "foo"


def test_sanitize_slug_path_traversal_neutralized(leerie):
    ...
    out = leerie._sanitize_slug("../../etc/passwd")
    assert ".." not in out
    assert "/" not in out
    assert out == "etc-passwd"


def test_sanitize_slug_strips_control_chars(leerie):
    """Control characters and newlines collapse to dashes."""
    out = leerie._sanitize_slug("task\nwith\tcontrol\x00chars")
    assert out == "task-with-control-chars"


def test_sanitize_slug_rejects_only_symbols(leerie):
    """All-symbols input has no usable characters — returns 'task' as a
    safe non-empty fallback rather than raising."""
    assert leerie._sanitize_slug("!!!@@@###") == "task"


def test_sanitize_slug_empty_string(leerie):
    assert leerie._sanitize_slug("") == "task"


def test_sanitize_slug_none_safe(leerie):
    """Defensive: None coerces to '' rather than crashing."""
    assert leerie._sanitize_slug(None) == "task"


def test_sanitize_slug_truncates_on_word_boundary(leerie):
    """Long input gets cut at the last `-` within max_len, so we never
    slice a word in half."""
    out = leerie._sanitize_slug(
...
def test_sanitize_slug_truncates_dashless_input(leerie):
    """If the input has no '-' within max_len (e.g., a single long token
    with all non-ASCII), the hard-truncate fallback applies."""
    out = leerie._sanitize_slug("a" * 80)
    assert len(out) <= 30


def test_sanitize_slug_respects_custom_max_len(leerie):
    """The `max_len` parameter can be overridden."""
    out = leerie._sanitize_slug("a-very-long-multi-word-task", max_len=10)
    assert len(out) <= 10


def test_sanitize_slug_unicode_becomes_dashes(leerie):
    """Non-ASCII characters are not transliterated — they're replaced
    with dashes (and then collapsed). Predictable over fancy."""
    out = leerie._sanitize_slug("héllo wörld")
...
def test_compute_run_id_uses_first_category(leerie):
...
def test_compute_run_id_skips_unknown_categories(leerie):
...
def test_compute_run_id_falls_back_to_misc(leerie):
    ...
    rid = leerie.compute_run_id([], "x", "2026-05-26T00:00:00+00:00")
    assert rid.startswith("misc-")
    rid = leerie.compute_run_id(["bogus"], "x", "2026-05-26T00:00:00+00:00")
    assert rid.startswith("misc-")


def test_compute_run_id_different_timestamps_different_ids(leerie):
    ...
    a = leerie.compute_run_id(["testing"], "x", "2026-05-26T14:31:22.847291+00:00")
    b = leerie.compute_run_id(["testing"], "x", "2026-05-26T14:31:22.847292+00:00")
    assert a != b


def test_compute_run_id_shortid_stable_per_timestamp(leerie):
