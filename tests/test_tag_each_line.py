...
def test_empty_content_returns_empty_string(leerie):
    assert leerie._tag_each_line("  [sid tool-fail]", "") == ""


def test_only_blank_lines_returns_empty_string(leerie):
    assert leerie._tag_each_line("  [sid tool-fail]", "\n\n\n") == ""


def test_single_line_renders_as_prefix_space_content(leerie):
    assert leerie._tag_each_line("  [sid tool-fail]", "boom") == "  [sid tool-fail] boom"


def test_multi_line_first_line_keeps_full_prefix(leerie):
    out = leerie._tag_each_line("  [sid tool-fail]", "line one\nline two\nline three")
    first = out.splitlines()[0]
    assert first == "  [sid tool-fail] line one"


def test_multi_line_continuation_drops_kind_and_pads_to_width(leerie):
    prefix = "  [sid tool-fail]"
    out = leerie._tag_each_line(prefix, "line one\nline two")
...
def test_multi_line_continuation_consistent_across_all_tail_lines(leerie):
    out = leerie._tag_each_line("  [sid tool-fail]", "a\nb\nc\nd")
...
def test_blank_interior_lines_are_filtered(leerie):
    out = leerie._tag_each_line("  [sid tool-fail]", "one\n\ntwo\n\n\nthree")
    lines = out.splitlines()
    assert lines == [
        "  [sid tool-fail] one",
        lines[1],
...
def test_works_with_tool_ok_kind(leerie):
    out = leerie._tag_each_line("  [sid tool-ok]", "x\ny")
    lines = out.splitlines()
...
def test_works_with_long_sid(leerie):
    prefix = "  [feat-002-conformer tool-fail]"
    out = leerie._tag_each_line(prefix, "row one\nrow two")
    lines = out.splitlines()
...
def test_defensive_fallback_on_unexpected_prefix_shape(leerie):
    # Prefix with no brackets falls back to repeating the prefix.
    out = leerie._tag_each_line("xx", "a\nb")
    assert out == "xx a\nxx b"


def test_defensive_fallback_on_prefix_without_kind_token(leerie):
    # Bracketed prefix but no space inside brackets → no kind to drop;
    # falls back to repeating the prefix verbatim on every line.
    out = leerie._tag_each_line("[sidonly]", "a\nb")
