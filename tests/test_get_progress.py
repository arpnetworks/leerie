...
def test_no_waves_returns_none(centella):
    ...
    assert centella._get_progress(st) is None


def test_empty_waves_returns_none(centella):
...
def test_all_pending_returns_zero_done(centella):
    ...
    assert centella._get_progress(st) == (0, 3)


def test_some_terminal_counted(centella):
    ...
    assert centella._get_progress(st) == (2, 3)


def test_all_terminal(centella):
    """All subtasks in a terminal state → done == total."""
    st = SimpleNamespace(data={
        "waves": [["a", "b"]],
        "subtask_status": {"a": "complete", "b": "blocked"},
    })
    assert centella._get_progress(st) == (2, 2)


def test_blocked_counted_as_terminal(centella):
    ...
    assert centella._get_progress(st) == (1, 1)
